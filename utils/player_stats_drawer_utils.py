...
def draw_player_stats(output_video_frames,player_stats):
    """
    Draws player statistics on video frames, including shot speed and movement speed.

    Parameters:
    - output_video_frames (list): List of video frames (numpy arrays).
    - player_stats (DataFrame): DataFrame containing player statistics per frame.

    Returns:
    - list: Updated video frames with overlaid player statistics.
    """

    for index, row in player_stats.iterrows():
        # Extracting stats for both players
        ...
        # Create semi-transparent background for the stats
        overlay = frame.copy()
